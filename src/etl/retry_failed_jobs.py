...
DESIGN NOTE on distinguishing "genuinely empty" from "transient failure"
(per F008's spec -- this is the whole point of the module): a crawler
signals genuine emptiness by raising EmptyResultError, and any other
exception is treated as transient and eligible for retry up to
max_retry. This is a convention the *caller* opts into (see run_job()) --
it is NOT automatically applied to F001/F002/F005/F006 as written. Those
crawlers currently raise plain ValueError for their own empty-fetch
guards (e.g. F002's normalize_ohlcv, F005's melt_pivoted_statement), which
run_job() would currently treat as a transient failure, not genuine
emptiness. Retrofitting those crawlers to raise EmptyResultError where
appropriate is a follow-up integration step, out of scope for this pass
-- flagging explicitly rather than silently leaving it inconsistent.
"""
...
def retry_all(
    ...
    """Re-run every retryable job for a given dataset. fn_factory(symbol)
    must return a zero-arg callable (so the caller can close over
    per-symbol args like date ranges). Returns {'succeeded': [...],
    'failed': [...], 'empty': [...]} symbol lists for this run.
    """
