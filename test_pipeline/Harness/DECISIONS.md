...
- Decision & Evidence:
  1. Process Termination: Gracefully stopped `run_all_sites_all_dates.py` (task-11998) and forcefully terminated PID 11356 to cleanly release all Windows file lock handles on `db/crawlers_staging.duckdb`. Verified 0 running crawler tasks/processes.
  2. Data Volume Highlights:
     - `tinnhanhchungkhoan`: Surged from 270 to **102,344 articles** (sitemaps swept from 2026-09 back to 2022-01-04 with 100% full text).
     - `tuoitre`: Swept pages 1 to 1,880 reaching **63,058 articles** down to 2010-08-04 across 4 zones (11, 89, 10, 3).
     - `crawlers_staging.duckdb`: Accumulated 152,945 validated articles with 100% full body preservation and ISO UTC timestamps.
  3. Database Sync & Merge:
     - Executed atomic DuckDB merge with idempotent deduplication (`ON CONFLICT (source_url) DO UPDATE`).
     - Main DB (`db/vesta.duckdb`): `core.macro_policy` expanded from 95,461 to **278,174 rows** (+182,713 new unique records).
     - Backup DB (`db/vesta_latest_backup.duckdb`): `core.macro_policy` expanded to **278,174 rows**; `core.pit_events` synced from 1,228 to **658,182 rows**. Both databases now 100% synchronized across all core tables.
     - Consolidated Snapshot (`db/vesta_consolidated.duckdb`): Created a 4,690 MB snapshot containing all 278K macro articles, 661K equity news, 5.17M OHLCV rows, and 658K PIT events.
  4. Total Corpus: Grand total of news & policy articles across VESTA reached **939,732 articles** (661,558 `core.news` + 278,174 `core.macro_policy`).

## 2026-09-10: Deep Historical Crawl Expansion (29,053 Articles: Tiền Phong & Tuổi Trẻ)
- Reason: User requested continuous backfill for Báo Tiền Phong (deep mode across 3 zones) and Báo Tuổi Trẻ (deep mode sweeping pages 881 to 1380 across 4 zones: 11, 89, 10, 3).
- Decision & Evidence:
  1. Handled Windows DuckDB lock contention (caused by IDE PID 14456 inspecting files) by upgrading `get_safe_db_connection()` to a 3-tier fallback architecture (`vesta.duckdb` -> `vesta_latest_backup.duckdb` -> `crawlers_staging.duckdb`).
  2. Báo Tiền Phong: Executed `src/crawlers/tienphong_crawler.py --mode deep --start-page 1 --max-pages 50 --zones 3,166,5`. Saved 3,000 articles across Zones 3 (Kinh tế), 166 (Địa ốc), and 5 (Thế giới).
  3. Báo Tuổi Trẻ: Executed `src/crawlers/run_tuoitre_after_tienphong.py --start-page 881 --max-pages 500 --zones 11,89,10,3`. Acquired lock immediately after Tiền Phong finished and crawled 500 pages down to **2010-08-04** (extending historical coverage to 16 continuous years). Saved **26,053 articles** with 100% full text body and ISO-8601 UTC timestamps.
  4. Total records in `crawlers_staging.duckdb`: 31,260 articles ready for merge into `core.macro_policy` via `src/etl/merge_crawlers_staging.py`. Maintained zero look-ahead bias and idempotent deduplication (`ON CONFLICT (source_url) DO NOTHING`).
  5. Next start page for Báo Tuổi Trẻ: **Page 1,381**.

## 2026-09-10: Modernization of General News Web Scraping Modules (`tienphong`, `tuoitre`, `unified_macro`)
- Reason: User requested updating the high-speed, deep crawled codes (discovered REST JSON API and timeline pagination streams) into the general news web scraping codebase.
- Decision & Evidence:
  1. Upgraded `src/crawlers/tienphong_crawler.py`: Replaced the legacy single-page XML sitemap parser with the multi-zone REST JSON API (`api.tienphong.vn/api/morenews-zone-...`), multi-threaded detail body extraction (`ThreadPoolExecutor`), multi-zone support (3: Kinh tế, 166: Địa ốc, 5: Thế giới), automatic DuckDB lock fallback, and `--start-page` / `--max-pages` controls while keeping backward-compatible `parse_tienphong_article` for test suites.
  2. Upgraded `src/crawlers/tuoitre_crawler.py`: Replaced the legacy 20-item RSS parser with the deep timeline stream engine (`tuoitre.vn/timeline/{zone}/trang-{page}.htm`) reaching back to 2009, 8 concurrent detail workers, ISO-8601 `<meta article:published_time>` UTC parsing, retry resilience, and per-page streaming commits.
  3. Integrated both into `src/crawlers/unified_macro_crawler.py`: Added active crawling of Tiền Phong and Tuổi Trẻ under Group A (Domestic Press & Economic Media).
  4. Verified: 29/29 tests in `tests/test_new_sector_crawlers.py` pass cleanly.
...
- Addendum (2026-09-09): each of the 16 regimes below tagged with a scope
  classification -- GLOBAL, VN_DOMESTIC, or BOTH. Purpose: lets a future
  analysis separate "does sentiment mean-reversion hold during VN-domestic-driven
  regimes vs. globally-driven regimes" rather than treating all 16 as
  interchangeable. Classification basis: whether the regime's proximate cause
  (per its cited sources) originated in Vietnam (VN_DOMESTIC), outside Vietnam
  with VN as a downstream transmission (GLOBAL or BOTH depending on whether
  VN's reaction was itself notable), or both drivers overlapped in the same
  window (BOTH). `src/pipeline/f201_robustness_check.py`'s `REGIME_BOUNDARIES`
  updated to a 4-tuple `(name, start, end, scope)` accordingly.

### Regime Scope Classification

| # | Regime | Scope | Reasoning |
|---|---|---|---|
| 1 | dotcom_bubble_crash_vn | **BOTH** | VN-specific speculative bubble mechanics, but coincided with the global Nasdaq dot-com crash |
| 2 | post_bubble_consolidation | **VN_DOMESTIC** | Low-liquidity VN sideways period; no major global driver identified |
| 3 | pre_gfc_bull_run | **VN_DOMESTIC** | Domestic retail-driven bull frenzy pre-2007; global credit boom was background, not the proximate cause |
| 4 | gfc_crash | **BOTH** | Global Financial Crisis (Lehman) was the cause; VN-Index crash was the local transmission |
| 5 | post_gfc_recovery | **BOTH** | Global monetary easing + VN domestic rebound |
| 6 | euro_debt_crisis_overlay | **GLOBAL** | European sovereign debt crisis / US credit downgrade — indirect sentiment overlay, no major VN-specific event in this window |
| 7 | steady_growth | **BOTH** | Mostly VN domestic growth; global oil-price glut (2014) as a minor overlay |
| 8 | bull_run_2016_2018 | **VN_DOMESTIC** | Domestic bull market, low-rate global backdrop was background, not the driver |
| 9 | bear_market_2018_2019 | **BOTH** | VN correction plus global US-China trade-war escalation |
| 10 | covid_crash | **BOTH** | Global pandemic was the cause; VN market impact was direct and severe |
| 11 | covid_recovery_rally | **BOTH** | Global stimulus/liquidity plus VN domestic retail-investor boom |
| 12 | real_estate_bond_crisis_2022 | **BOTH** (VN-dominant) | Primary driver was VN-domestic (FLC, Tân Hoàng Minh, Vạn Thịnh Phát arrests); Russia-Ukraine war and global rate hikes were a secondary overlay |
| 13 | recovery_2023_2024 | **VN_DOMESTIC** | Driven by VN policy relief (Decree 08/2023), not a global event |
| 14 | tariff_shock_ftse_rally_2025 | **BOTH** | Global Trump-tariff shock, then VN-specific FTSE-upgrade anticipation and GDP growth |
| 15 | ftse_upgrade_correction | **VN_DOMESTIC** | FTSE reclassification is VN-specific; the Oct 20 correction was a local shakeout, not a global selloff |
| 16 | pre_upgrade_run_2026 | **BOTH** | Middle East/Iran-war tension as a global overlay; FTSE upgrade confirmation is VN-specific |
...
## 2026-09-11: Literature Review Integration, F202 Robustness Gate, and F203 Regime Gate Gating F301

- Reason: Synthesis of 2 rounds of literature review (Bailey & López de Prado DSR/PBO 2014, PhoBERT VN price-reaction findings from Preprints.org, FinDPO arXiv:2507.18417, UBCKNN 09/2023 directive, and HybridACD TCD framework).
  Crucial insight: F201's Cohen's d = 0.0557 is classified as "negligible" (d < 0.2) by standard Cohen conventions. At n = 15,081, p-value = 8.389e-12 can easily result purely from huge sample size without demonstrating true economic edge.
- Decisions & Executed Actions:
  1. **F202 Refinement & Accurate Scoping**:
     Renamed F202 to "TIER GATE: Cluster-robust standard errors & regime heterogeneity audit" to avoid overclaiming DSR/PBO completion before formal formula implementation.
     Fixed `src/pipeline/f201_robustness_check.py`: integrated `src/pipeline/sentiment_lexicon.py`, fixed SQL column selection (`headline`, `price_at_publish IS NOT NULL`), created unit tests (`tests/test_f201_robustness.py`, 5/5 passing).
  2. **Mathematical Verification of p-value Discrepancy**:
     The slight difference between F201's report (`p = 8.3892e-12`) and F202's printout (`p = 8.0788e-12`) was mathematically proven to be 100% bit-identical in underlying data ($n = 15,081$, $t = 6.837140155087911$):
     - `backtest_meanreversion.py` used exact Student's t survival function via regularized incomplete beta with $df = 15,080$ ($p = 8.3892 \times 10^{-12}$).
     - `f201_robustness_check.py` used standard normal CDF survival function via Gaussian error function `erf` ($p = 8.0788 \times 10^{-12}$).
     There is zero data drift or randomness; the difference is purely Student-t tail heaviness vs Gaussian tail.
  3. **Executed Live Run against `db/vesta.duckdb` (core.pit_events)**:
     - **Pooled Naive Paired t-test**: n = 15,081, mean_diff = +1.8745% (+0.018745), SE = 0.00274, t = 6.8371, Cohen's d = 0.0557.
     - **Cluster Bootstrap by Symbol** (1,437 clusters, 2,000 iterations): SE_boot = 0.003135, 95% CI = [0.01327, 0.02518], z = 5.9789, p = 2.25e-09. 95% CI strictly excludes zero.
     - **Block Bootstrap by Month** (214 clusters, 2,000 iterations): SE_boot = 0.006018, 95% CI = [0.00695, 0.03049], z = 3.1151, p = 0.00184. 95% CI strictly excludes zero.
     - **Trial Count Audit Nuance**: Git history confirms trial count = 1 on `sentiment_lexicon.py` (starter dictionary committed once, no lexicon-level tuning). However, this does not rule out data snooping at the horizon ($t+5$ vs $t+30$) or threshold level across the wider pipeline; full pipeline trial auditing is partitioned into F202b.
  4. **Sign-Flip Detection & Hard Gate F203**:
     `sign_flip_detected = True` revealed severe magnitude:
     - 2022 real estate bond crisis: `mean_diff = -5.14%` (prices plunged further after bad news, zero mean reversion).
     - FTSE upgrade correction: `mean_diff = -2.47%`.
     - COVID recovery rally: `mean_diff = +6.74%`.
     **Critical Methodological Decision**: This regime heterogeneity is treated as a major risk, NOT a minor footnote. Training PhoBERT (F301) on pooled data risks instilling a "catch-the-falling-knife" bias that blows up during liquidity crises.
     Created **F203** ("Regime-conditional validity audit") as an explicit hard gate blocking F301 until resolution (regime gating input feature, down-weighting crisis regimes, or execution-level regime circuit breakers).
  5. **Added F202b**: Formal DSR/PBO calculation across whole pipeline trials per Bailey & López de Prado (2014).
  6. **Updated F301 Dependencies**: `["F202", "F202b", "F203"]`. F301 is strictly blocked until F202b and F203 pass.
  7. **Updated F901 Evidence**: Documented UBCKNN Sept 2023 directive banning automated robot order placement; remains `blocked`.
  8. **Added F303**: HybridACD TCD consistency check adapted to softmax classification head.
...
## 2026-09-12: F202b DSR / PBO Outlier Diagnostic & Methodological Calibration
- Reason: The mathematical verification of DSR at N=1 (0.9542) and N=2 (0.8983) matched 100%, but an independent audit uncovered three critical methodological vulnerabilities requiring resolution before thesis submission:
  1. **Extreme Kurtosis (4,315.77)**: Extreme non-normality 1,438x above Gaussian (3.0) suggested higher moments in DSR were completely dominated by data outliers, potentially producing a garbage-in-garbage-out false failure.
  2. **Borderline N=1 Interpretation**: DSR = 0.9542 was only 0.0042 above 0.95; sensationalist narrative tone in previous logs was uncalibrated to this razor-thin margin.
  3. **Methodological Adaptation of Cohen's d**: Bailey & Lopez de Prado (2014) formulated DSR for time-series Sharpe Ratio, not cross-sectional event-study Cohen's d. This adaptation requires explicit documentation in the thesis Method section.
- Diagnostic Findings:
  1. **Outlier Root Cause Identified**: Querying top 20 extreme diffs (|return_t30 - return_t5|) revealed that 97% of excess kurtosis was caused by a SINGLE UPCOM manipulation event: `XDC` on 2023-05-12 (price soared from 31.3k to 999.9k VND, +3,095% return, diff = +30.21). Excluding this single row collapsed Kurtosis from 4,315.77 to 126.87.
  2. **Exchange & Penny Stock Concentration**: The remaining top 19 outliers were exclusively UPCOM/HNX penny stocks (<3,000 VND) or delisting events (PTM, BTH, POM, BBT) where prices jumped upon transfer to UPCOM. Zero outliers originated from HOSE (where Kurtosis was naturally 18.48).
  3. **Inflated Variance Deflated Cohen's d**: The unadjusted XDC outlier inflated sample SD from 17.0% to 33.7%, which artificially suppressed raw Cohen's d to 0.0557.
- Resolution & Verified Evidence:
  1. **Robust DSR Across Outlier Treatments**:
     - *Raw Baseline*: Kurtosis = 4,315.77, Skew = 49.94, Cohen's d = 0.0557 -> DSR(N=1)=0.9543 (PASS), DSR(N=2)=0.8983 (FAIL).
     - *Exclude XDC Only*: Kurtosis = 126.87, Skew = 6.69, Cohen's d = 0.0728 -> DSR(N=1)=0.9996 (PASS), DSR(N=2)=0.9967 (PASS), DSR(N=3)=0.9896 (PASS).
     - *Winsorized [0.5%, 99.5%]*: Kurtosis = 10.84 (normal financial level), Skew = 1.72, Cohen's d = 0.0729 -> DSR(N=1)=0.9981 (PASS), DSR(N=2)=0.9914 (PASS), DSR(N=3)=0.9767 (PASS).
     - *Winsorized [1.0%, 99.0%]*: Kurtosis = 6.98, Skew = 1.17, Cohen's d = 0.0675 -> DSR(N=1)=0.9955 (PASS), DSR(N=2)=0.9829 (PASS), DSR(N=3)=0.9617 (PASS).
     - *HOSE Main Board (Raw)*: Kurtosis = 18.48, Skew = 2.13, Cohen's d = 0.0994 -> DSR(N=1)=0.9852 (PASS), DSR(N=2)=0.9455 (FAIL), DSR(N=3)=0.8920 (FAIL).
     - *HOSE Main Board (Winsorized 0.5%)*: Kurtosis = 6.37, Skew = 1.06, Cohen's d = 0.0984 -> DSR(N=1)=0.9802 (PASS), DSR(N=2)=0.9348 (FAIL), DSR(N=3)=0.8778 (FAIL).
  2. **Critical Market Feasibility & Statistical Finding**: While the pooled universe passes DSR across all candidate horizons $N \in [1, 2, 3]$ under standard winsorization, HOSE-only fails DSR at $N \ge 2$ ($DSR = 0.935$ at $N=2$, $0.878$ at $N=3$) due to smaller effective cluster count ($T=398$) and narrower reversion spread. This proves mathematically that the apparent alpha is disproportionately driven by smaller, illiquid UPCOM/HNX stocks — presenting a dual hazard (statistical fragility on large caps, execution/slippage barrier on small caps).
  3. **Requirement for F203 (Regime Audit)**: F203 must formally evaluate mean-reversion with exchange segmentation (HOSE vs UPCOM/HNX) alongside temporal regimes, and integrate international liquidity/macro controls (^VIX, DX-Y.NYB, ^TNX from core.market_index_daily).
  4. **Trial Count Upper Bound**: Audited schema of `core.pit_events` has strictly 3 price horizon columns (`price_t1`, `price_t5`, `price_t30`), restricting testable horizon pairs to $N \le 3$.
